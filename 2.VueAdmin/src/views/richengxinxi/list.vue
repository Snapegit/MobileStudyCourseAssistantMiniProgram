	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.zhouci" 
								placeholder="周次"
								>
								<el-option v-for="item in zhouciLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('richengxinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('richengxinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('richengxinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('richengxinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('richengxinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.zhouci&&searchQuery.value.zhouci!=''){
			params['zhouci'] = searchQuery.value.zhouci
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	const getzhouciLists = () => {
		zhouciLists.value = '周一,周二,周三,周四,周五,周六,周日'.split(',')
	}
	getzhouciLists()
	const searchClick = () => {
